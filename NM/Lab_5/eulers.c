...
int main() {
    ...
    for (i = 0; i < n; i++) {
        m1 = f(x0, y0);         // Calculate the value of m1 (f(x, y))
        yn = y0 + h * m1;       // Update the value of y using Euler's method
        xn = x0 + h;            // Update the value of x
        printf("%4d | %8.2f | %8.4f | %8.4f | %8.4f\n", i + 1, x0, y0, m1, yn);
